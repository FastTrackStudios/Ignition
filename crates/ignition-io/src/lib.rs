//! The wire. Everything above resolves to one byte per channel per
//! universe; this crate is how those bytes leave the machine — sACN
//! (E1.31) and Art-Net 4, per universe, at a bounded rate — and how the
//! visualizer is handed the *same* bytes (`Sink`) so what is on screen is
//! what left the socket.
//!
//! ```text
...
fn default_max_hz() -> f32 {
    44.0
}
fn default_keepalive_hz() -> f32 {
    1.0
}
    ...
    pub fn send(&self, frame: &HashMap<u16, [u8; 512]>, now: Instant) {
        ...
        for (number, cfg) in &self.config.universes {
            ...
            if let (Some(art), Some(sock)) = (&cfg.artnet, &self.artnet_socket) {
                let pkt = artnet::art_dmx(state.artnet_seq, art.port_address(), data);
...
mod tests {
    ...
    fn config_deserialises_the_venue_shape_with_defaults_and_round_trips() {
        ...
        assert_eq!(a.port_address(), PortAddress::new(0, 0, 0));
        assert!(a.broadcast);
