...
struct InitOutput {
    ...
    /// IGNITION PATCH: whether this init made a new device (which then
    /// joins the shared pool) or reused one from it.
    created_device: bool,
}
...
fn build_wgpu_context(config: &VelloRendererOptions) -> WGPUContext {
    ...
    let mut ctx = WGPUContext::with_features_and_limits(Some(features), config.limits.clone());
    // IGNITION PATCH: every renderer in the process draws through one
    // wgpu `Instance`, so a device made for the first window's surface
    // is compatible with every later window's surface too.
    ctx.instance = SHARED_INSTANCE.get_or_init(|| ctx.instance.clone()).clone();
    ctx
}

// IGNITION PATCH (multi-window): one `Instance` and one device pool for
// the whole process. Upstream gives each `VelloWindowRenderer` its own
// `WGPUContext`, so a second window means a second `Device` — and a
// texture rendered by Bevy on the first window's device cannot be
// painted into the second. Sharing the pool means the second window
// finds the first window's device (`is_surface_supported`) and reuses
// it; there is exactly one device per adapter in the process.
static SHARED_INSTANCE: std::sync::OnceLock<wgpu::Instance> = std::sync::OnceLock::new();
static SHARED_DEVICES: std::sync::Mutex<Vec<DeviceHandle>> = std::sync::Mutex::new(Vec::new());

/// Every device any Vello window renderer in this process has created,
/// in creation order. What the embedder reads to prove that two windows
/// share one device.
pub fn shared_device_handles() -> Vec<DeviceHandle> {
    SHARED_DEVICES.lock().map(|d| d.clone()).unwrap_or_default()
}
...
impl WindowRenderer for VelloWindowRenderer {
    ...
    fn resume<F: FnOnce() + 'static>(
        ...
        // IGNITION PATCH: look in the process-wide pool, not only this
        // renderer's own.
        if let Ok(shared) = SHARED_DEVICES.lock() {
            self.wgpu_context.device_pool = shared.clone();
        }
        ...
        spawn_init(async move {
            let created_device = existing_device_handle.is_none();
        ...
        match &mut self.render_state {
            ...
            RenderState::Pending { receiver } => match receiver.try_recv() {
                Ok(Some(InitOutput {
                    ...
                    // IGNITION PATCH: a newly made device joins the
                    // process-wide pool so the next window reuses it.
                    if created_device && let Ok(mut shared) = SHARED_DEVICES.lock() {
                        shared.push(device_handle.clone());
                    }
