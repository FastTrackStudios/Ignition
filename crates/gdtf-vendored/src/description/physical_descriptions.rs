...
define_collect_helper!(mod serialize_emitters "Emitter" -> crate::physical_descriptions::Emitter);
...
define_collect_helper!(mod serialize_filters "Filter" -> crate::physical_descriptions::Filter);
...
define_collect_helper!(mod serialize_color_spaces "ColorSpace" -> crate::physical_descriptions::ColorSpace);
...
define_collect_helper!(mod serialize_gamuts "Gamut" -> crate::physical_descriptions::Gamut);
...
mod serialize_gamut_points {
    use crate::values::ColorCie;
    use serde::de::{Error, Unexpected, Visitor};
    use serde::{Deserializer, Serializer};
    use std::fmt::{Display, Formatter};
    use std::str::FromStr;

    pub fn serialize<S>(value: &[ColorCie], serializer: S) -> Result<S::Ok, S::Error>
    where
        S: Serializer,
    {
        struct GamutDisplay<'a>(&'a [ColorCie]);

        impl<'a> Display for GamutDisplay<'a> {
        ...
        serializer.collect_str(&GamutDisplay(value))
    }

    pub fn deserialize<'de, D>(deserializer: D) -> Result<Vec<ColorCie>, D::Error>
    where
        D: Deserializer<'de>,
    {
        ...
        impl<'de> Visitor<'de> for PointsVisitor {
            type Value = Vec<ColorCie>;

            fn expecting(&self, formatter: &mut Formatter) -> std::fmt::Result {
                formatter.write_str("a points string in the format {x,y,Y}{x,y,Y}...")
            }
                ...
                E: Error,
            {
                ...
                while !v.is_empty() {
                    ...
                    let x = f64::from_str(x).map_err(|_| {
                        E::invalid_value(Unexpected::Str(x), &"a floating point number")
                    })?;
                    let y = f64::from_str(y).map_err(|_| {
                        E::invalid_value(Unexpected::Str(y), &"a floating point number")
                    })?;
                    let z = f64::from_str(z).map_err(|_| {
                        E::invalid_value(Unexpected::Str(z), &"a floating point number")
                    })?;

                    points.push(ColorCie { x, y, z });
...
define_collect_helper!(mod serialize_dmx_profiles "DMXProfile" -> crate::physical_descriptions::DmxProfile);
...
define_collect_helper!(mod serialize_cri_groups "CRIGroup" -> crate::physical_descriptions::CriGroup);
...
define_collect_helper!(mod serialize_connectors "Connector" -> crate::physical_descriptions::Connector);
...
#[cfg(test)]
mod tests {
    use crate::physical_descriptions::Gamut;
    use crate::values::ColorCie;

    #[test]
    fn parse_three_point_gamut() {
        let xml = r#"<Gamut Name="Test" Points=" { 0.640000 , 0.330000 , 21.262754 } { 0.300000 , 0.600000 , 71.516600 } { 0.150000 , 0.060000 , 7.224500 } "/>"#;
        let g: Gamut = quick_xml::de::from_str(xml).expect("parse");
        assert_eq!(
            g.points,
            &[
                ColorCie {
    ...
    #[test]
    fn empty_points_attribute_parses_to_empty_vec() {
        let xml = r#"<Gamut Name="Empty" Points=""/>"#;
        let g: Gamut = quick_xml::de::from_str(xml).expect("parse");
        assert!(g.points.is_empty());
    }

    #[test]
    fn invalid_gamut_points_fails() {
        let xml = r#"<Gamut Name="Test" Points="{ 0.640000 , bad , 21.262754 }"/>"#;
        let res = quick_xml::de::from_str::<Gamut>(xml);
        assert!(res.is_err());
    }
}
