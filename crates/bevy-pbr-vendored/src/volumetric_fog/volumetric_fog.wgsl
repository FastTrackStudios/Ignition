...
// IGNITION PATCH: shared with the froxel injection — see
// `volumetric_shadows.wgsl`.
#import bevy_pbr::volumetric_shadows::{
...
fn fragment(@builtin(position) position: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    for (var step = 0u; step < step_count; step += 1u) {
            ...
            i = i + 1u)
        {
            ...
            // IGNITION PATCH: a lamp has a lens, so the inverse square
            // stops at its surface.
            //
            // Upstream takes the raw distance, so radiance runs to
            // infinity at the light's position and a cone is a
            // singularity at its apex. On a surface that is invisible —
            // nothing is ever *at* the light. In fog it is the brightest
            // thing on screen, packed into less than one pixel of the
            // haze buffer, and it comes out as a block. Widen the cone
            // and the blocks stop, which is the tell: a real fixture's
            // beam leaves a lens a hundred and fifty millimetres across,
            // not a point.
            //
            // Clamping the distance to the light's own radius is the
            // standard sphere-light form of this, and Bevy already
            // uploads that radius in `position_radius.w` — the fog is
            // simply the one place that ignores it. A light with radius
            // zero is unchanged, so nothing that does not set one moves.
            let lens_radius = (*light).position_radius.w;
            let distance_square = max(
                dot(light_to_frag, light_to_frag),
