...
pub fn prepare_lights(
    ...
    // IGNITION PATCH: likewise — see `spot_light_volumetric_enabled_count`.
    ...
    // IGNITION PATCH: not capped by shadow-map array capacity — a
    // volumetric light needs no map. See the flag below.
    ...
    for (index, entity) in point_light_entities.iter().enumerate() {
        ...
        // IGNITION PATCH: a light may scatter in the fog without
        // casting a shadow.
        //
        // Upstream requires `shadow_maps_enabled` here, and the shader
        // skips any light without the flag — so "in the air" and "casts
        // a shadow" are the same switch. For a lighting visualiser they
        // are not: a shadow map is a render pass per light per frame and
        // the budget affords eight, while *every* fixture in a hazed
        // room shows its cone in the air. Sixty-seven emitters were
        // marked volumetric and eight were, which is not what
        // `r[viz.haze-is-volumetric]` says and not what a room looks
        // like.
        //
        // Nothing else has to change: `volumetric_fog.wgsl` already
        // defaults `shadow = 1.0` for a light whose shadow bit is clear,
        // so a shadowless light scatters correctly and simply
        // silhouettes nobody. The counts above are no longer bounded by
        // shadow-map capacity either, for the same reason — a light that
        // wants no map should not compete for one.
        //
