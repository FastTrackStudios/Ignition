...
impl<'doc> DocumentMutator<'doc> {
    ...
    fn process_removed_subtree(&mut self, node_id: usize) {
        self.doc.iter_subtree_mut(node_id, |node_id, doc| {
            ...
            // IGNITION PATCH: the text selection and the mousedown node are
            // node ids too. Leaving them pointing at a removed node made the
            // next selection update walk a detached subtree and panic in
            // `compare_document_order`; a dropped node would hit a slab
            // index panic instead. Clear them like hover/active above.
