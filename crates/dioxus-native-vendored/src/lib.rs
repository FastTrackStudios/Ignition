...
pub use blitz_shell::{BlitzShellEvent, BlitzShellProxy};
pub use dioxus_application::{DioxusNativeApplication, DioxusNativeEvent, WindowFactory};
...
pub use hooks::{
    WindowOpen, close_window, close_window_via, open_window, open_window_via,
    open_window_with_props, use_back_button, use_shell_proxy, use_window_event,
};
pub use winit;
...
use blitz_shell::{WindowConfig, create_default_event_loop};
...
pub fn launch_cfg_with_props<P: Clone + 'static, M: 'static>(
    ...
    #[cfg(any(feature = "vello", feature = "vello-hybrid"))]
    let renderer = DioxusNativeWindowRenderer::with_features_and_limits(features, limits.clone());
    ...
    // IGNITION PATCH: what later windows are built from. The launch
    // window's providers are shared; `Arc`s all the way down.
    ...
    // Create application
    let application =
        DioxusNativeApplication::new(proxy, event_queue, config).with_window_factory(factory);
