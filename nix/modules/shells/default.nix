  ...
  perSystem = { pkgs, lib, config, ... }: {
    devShells.default = pkgs.mkShell ({
      ...
      shellHook = ''
        ...
        echo ""
        echo "  Ignition dev shell"
        echo "  ─────────────────────────────────────────────"
        echo "  just check / just test / just ci"
        echo "  cargo run -p ignition-engine — the lighting engine"
        echo "  cd apps/ignition-mobile && dx serve   — the Ignition app"
        echo "  (cd apps/ignition-mobile && ./ios/build-ios.sh)   — the iOS app"
        echo ""
