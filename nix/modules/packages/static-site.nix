...
# on :8080, SPA-fallback unknown paths to index.html.
#
# The fallback is not a nicety. The site is a client-side router, so
# `/guide/the-song` is a route and not a file — without it every deep
# link and every refresh is a 404 from the server, which is exactly what
# a plain static host does.
#
# HTML is `no-cache` (nix-store mtimes are 1970 — heuristic freshness
# would pin index.html for months); /assets/** is immutable-forever,
# because dx content-hashes every asset URL. Real copy, not symlinks:
...
#   $(nix build --print-out-paths .#ignition-web-image) \
#     | skopeo copy docker-archive:/dev/stdin docker://…
# which is what keeps a multi-megabyte bundle out of the store twice.
