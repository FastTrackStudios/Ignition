...
- **Fixture profiles (GDTF)**: downloaded manufacturer `.gdtf` files live in `data/gdtf/`; `data/gdtf/generated/` holds profiles built by `python3 tools/make_gdtf.py --all` from `data/gdtf/specs/*.json` (base geometry per `tools/gdtf_bases.json`; the generated FixtureType `Name` is the spec's `console_name`, which must be unique and equal the venue's model string). `GdtfLibrary::load_default()` (`crates/ignition-viz/src/gdtf_geometry.rs`) loads both directories, generated winning on a name clash, and `find(manufacturer, model)` matches exact normalized name, then `data/gdtf/aliases.json` (`{"<venue model string>": "<FixtureType Name>"}`), then fuzzy substring. `cargo test -p ignition-viz --test gdtf_coverage` proves every venue fixture resolves to a profile with a real mesh — add an alias when it fails.
